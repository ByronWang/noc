            ...
            <tr>
				...
				<td>[@input field=f parent="item" /]</td>
					[#elseif f.inline][#t]
       				[#elseif f.refer][#t]
                    	<#if item.${f.name}??>[#t]
				<td>[#if f.type.keyField?? ]${r"${item." + f.name + "." + f.type.keyField.name + "}"}[#else]ERROR[NO KeyField][/#if] <!--[link typename=f.type.name field=f.type.keyField parent="item."+f.name /]--></td>
                    	<#else>[#t]
				<td>refer</td>
						</#if>[#t]
			     	[#else][#t]
...
<ol>[#list type.fields as f]
	<li>[#if f.array && f.inline ]   
		[@label field=f/][@typeinfo tp=f.type /]		
		[@list  field=f /]
	[#elseif f.array]
 		[@label field=f /][@typeinfo tp=f.type /]   
		[@list  field=f /]
